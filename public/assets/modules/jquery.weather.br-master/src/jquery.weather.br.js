/*
 *  jquery.weather.br - v1.0.0
 *  Plugin jQuery do clima brasileiro.
 *  http://lab.thiagosr.com.br/jquery.weather.br
 *
 *  Made by Thiago Resende
 *  Under MIT License
 */
// i18next, v1.8.0
...
;(function ($, window, document, undefined) {
	...
	var getXML = function (url, callbackSuccess, callbackError) {
		...
		var method = 'GET';
		var app_id = 'vny03B76';
		var consumer_key = 'dj0yJmk9T01CR0szaUVuc3I2JnM9Y29uc3VtZXJzZWNyZXQmc3Y9MCZ4PTJl';
		var consumer_secret = 'b5baa9969b04614b9ac82e62f4e3a0a3298c61bf';
		var concat = '&';
		var query = {
	...
	var showWeather = function (weather) {
		...
		if (forecasts.length > 0) {
			// Set informations
			htmlWeather.find('h1 .city').text(weather.location.city);
			htmlWeather.find('h1 .region').text(weather.location.region);

			if (settings.format.toLowerCase() === 'week') {
				htmlWeather.addClass('week');
				var forecast;
				$.each(forecasts, function (index) {
					if(index < 7){
						forecast = htmlForecast.clone();
						forecast.find('.day').attr('data-i18n', this.day);
						forecast.find('.day').text(getDay(this.day));
						forecast.find('.wi').addClass(forecastIcons[this.code]);
						forecast.find('.temperature .low').html(getTemperature(this.low) + '°');
						forecast.find('.temperature .high').html(getTemperature(this.high) + '°');
				...
				htmlForecast.find('.temperature .low').html(getTemperature($(forecasts[0]).find('minima').text()) + '°');
				htmlForecast.find('.temperature .high').html(getTemperature($(forecasts[0]).find('maxima').text()) + '°');
