...
export default {
    ...
    methods: {
        ...
        updateUser() {
            ...
            _this.message = '';
            _this.loading = true;
            axios.post(this.$parent.MakeUrl('painel/users/'+this.getUserdata('id')), {'name': this.name, 'email': this.email, 'password': this.password, 'password_confirmation': this.password_confirmation, '_method': 'PATCH'}).then((res) => {
                _this.loading = false;
                _this.message = 'Informações do perfil atualizadas com sucesso!';
            }).catch((err) => {
