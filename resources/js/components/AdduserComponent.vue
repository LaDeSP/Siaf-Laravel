...
export default {
    ...
    methods: {
        getRoles() {
            axios.get(this.$parent.MakeUrl('painel/users/roles')).then((res) => {
                this.roles = res.data;
            }).catch((err) => {
        ...
        addUser() {
            ...
            _this.message = '';
            _this.loading = true;
            axios.post(this.$parent.MakeUrl('painel/users'), {'name': this.name, 'role': this.role, 'email': this.email, 'current_password': this.current_password, 'password': this.password, 'password_confirmation': this.password_confirmation}).then((res) => {
                _this.loading = false;
                _this.resetForm();
